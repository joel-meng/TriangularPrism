...
public extension HSVColor {
    ...
    init(
        ...
        @Clamped(0 ... 1) valueUnit: Float
    ) {
        hue = UInt8(Float(UInt8.max) * max(0, min(hueUnit, 1)))
        saturation = UInt8(Float(UInt8.max) * max(0, min(saturationUnit, 1)))
        value = UInt8(Float(UInt8.max) * max(0, min(valueUnit, 1)))
    }
}
